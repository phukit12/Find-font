...
# =========================================================================
# 🎯 [ULTIMATE PATCH] นำเข้าไลบรารีและสร้างระบบป้องกันไฟล์พัง
# =========================================================================
import timm
import omegaconf
...
class DummyResolver:
    ...
    @property
    def dict(self): return {} # ดักจับ error 'Resolver' object has no attribute 'dict'

class SafeUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if 'Resolver' in name or name == 'Resolver': return DummyResolver
        try:
            return super().find_class(module, name)
        except Exception:
            if 'omegaconf' in module or 'timm' in module: return DummyResolver
            raise

class SafePickle:
    Unpickler = SafeUnpickler
    @staticmethod
    def load(file, **kwargs): return SafeUnpickler(file, **kwargs).load()
    loads = pickle.loads
    dump = pickle.dump
    dumps = pickle.dumps
...
def load_model():
    ...
    if MODEL_PATH.exists() and MODEL_PATH.stat().st_size > 10240:
        # บังคับใช้ SafePickle หลบการเช็คเวอร์ชัน
...
with col_left:
    ...
    # แก้ไขแจ้งเตือน label ว่างเปล่าตรงนี้ครับ (ใส่ข้อความแล้วสั่งซ่อน)

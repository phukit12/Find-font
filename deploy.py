...
.tff-page { background: #f0ead8; min-height: 100vh; }
...
with col_left:
    st.markdown('<div style="padding: 1rem 0 0.5rem; font-family: Sarabun, sans-serif;">', unsafe_allow_html=True)
    uploaded_file = st.file_uploader("นำเข้ารูปภาพ", type=["jpg", "jpeg", "png", "webp", "bmp"])
    
    if uploaded_file:
        image = Image.open(uploaded_file)
        image = ImageOps.exif_transpose(image)
        st.markdown('<div class="tff-preview-box">', unsafe_allow_html=True)
...
with col_right:
    st.markdown('<div style="padding: 1rem 0 0;">', unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        def pct_class(p):
            ...
            if p >= 0.6:  return "tff-pct-mid"
            return "tff-pct-low"

        def bar_class(p):
            if p >= 0.85: return "tff-bar-green"
            if p >= 0.6:  return "tff-bar-yellow"
            return "tff-bar-gray"
